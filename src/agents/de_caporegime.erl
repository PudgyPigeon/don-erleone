...
-spec handle_conn_check({ok, state()} | {error, term()}, map()) -> {reply, term(), state()}.
...
handle_conn_check({error, Reason}, Spec) ->
  ...
  finalize_mission(Spec, Result),
  {reply, Result, undefined}. %% Returning undefined as state to force reconnect next time
...
-spec establish_mcp_conn(state()) -> {ok, state()} | {error, term()}.
establish_mcp_conn(#de_caporegime_state{config = Conf} = State) ->
  {Host, Port} = parse_mcp_url(de_config:sub_config_mcp_url(Conf)),
...
-spec wait_for_mcp_up(pid(), state()) -> {ok, state()} | {error, term()}.
wait_for_mcp_up(Conn, State) ->
  handle_await_up(gun:await_up(Conn, 10000), Conn, State).

-spec handle_await_up({ok, atom()} | {error, term()}, pid(), state()) -> {ok, state()} | {error, term()}.
...
-spec run_mission(map(), state()) -> {ok, map()} | {error, term()}.
run_mission(Spec, State) ->
  handle_discovery(discover_tools(State), Spec, State).

-spec handle_discovery({ok, list()} | {error, term()}, map(), state()) -> {ok, map()} | {error, term()}.
...
-spec discover_tools(state()) -> {ok, list()} | {error, term()}.
discover_tools(State) ->
  handle_tool_list(mcp_call(<<"tools/list">>, #{}, State)).
...
-spec recursive_loop(binary(), list(), list(), state(), integer()) -> {ok, map()} | {error, term()}.
recursive_loop(Prompt, Context, Tools, State, Step) ->
  Max = de_config:sub_config_max_steps(State#de_caporegime_state.config),
  execute_loop_step(Step, Max, Prompt, Context, Tools, State).

-spec execute_loop_step(integer(), integer(), binary(), list(), list(), state()) ->
  {ok, map()} | {error, term()}.
...
-spec process_llm_response(map(), list(), list(), state(), integer()) -> {ok, map()} | {error, term()}.
process_llm_response(Msg, Context, Tools, State, Step) ->
  handle_loop_decision(de_agent_brain:analyze_loop_step(Msg), Msg, Context, Tools, State, Step).

-spec handle_loop_decision({stop, binary()} | {continue, list()}, map(), list(), list(), state(), integer()) ->
  {ok, map()} | {error, term()}.
...
-spec check_continuation(list(), list(), state(), integer()) -> {ok, map()} | {error, term()}.
check_continuation(_NextCtx, _Tools, State, Step) ->
  Max = de_config:sub_config_max_steps(State#de_caporegime_state.config),
  decide_continuation(Step >= Max, _NextCtx, _Tools, State, Step).
...
-spec call_ollama(binary(), list(), list(), state()) -> {ok, map()} | {error, term()}.
...
-spec format_tool_response(binary(), binary(), binary()) -> map().
...
-spec safe_jsx_decode(binary()) -> {ok, term()} | {error, decode_failed}.
safe_jsx_decode(Raw) ->
...
format_content_texts([]) -> <<"No text response from tool.">>;
format_content_texts(Texts) -> iolist_to_binary(lists:join(<<"\n">>, Texts)).

%% =============================================================================
%% HTTP Transport
%% =============================================================================

-spec mcp_call(binary(), map(), state()) -> {ok, binary()} | {error, term()}.
mcp_call(Method, Params, #de_caporegime_state{config = Conf, conn = Conn}) ->
  ...
  #{path := Path} = uri_string:parse(de_config:sub_config_mcp_url(Conf)),
  Ref = gun:post(Conn, Path, Headers, Payload),
  await_gun_response(Conn, Ref, de_config:sub_config_timeout(Conf)).

-spec await_gun_response(pid(), reference(), integer()) -> {ok, binary()} | {error, term()}.
await_gun_response(Conn, Ref, Timeout) ->
  handle_gun_await(gun:await(Conn, Ref, Timeout), Conn, Ref, Timeout).
...
-spec finalize_mission(map(), {ok, map()} | {error, term()}) -> ok.
