...
-export([process_ndjson/3, finalize/3]).

%% =============================================================================
%% Pure Logic: JSON Stream Processing
%% =============================================================================

-spec process_ndjson(binary(), term(),
                     function() | undefined) -> {binary(), term()}.
...
-spec finalize(binary(), term(),
               function() | undefined) -> term().

finalize(Line, Acc, CB) ->
    handle_line_decode(de_ollama_brain:decode_line(Line),
                       Acc,
                       CB).

-spec handle_line_decode({ok, map()} |
                         {error, term()} |
                         skip,
                         term(), function() | undefined) -> term().

handle_line_decode({ok, Msg}, Acc, CB) ->
    de_ollama_brain:accumulate(Acc, Msg, CB);
handle_line_decode({error, Reason}, _Acc, _CB) ->
    {error, Reason};
handle_line_decode(skip, Acc, _CB) -> Acc.
