...
-spec analyze_llm_response(binary(),
                           list()) -> {delegate, binary(), map(), binary()} |
                                      {direct, binary()}.
...
analyze_llm_response(RawBin, _PrevContext) ->
    ...
    DelegateRaw = maps:get(<<"delegate_required">>,
                           Parsed,
                           false),
    Delegate = DelegateRaw =:= true orelse
                   DelegateRaw =:= <<"true">> orelse
                       DelegateRaw =:= "true",
    Intent = maps:get(<<"tool_intent">>,
                      Parsed,
                      <<"autonomous">>),
    Args = maps:get(<<"mcp_args">>, Parsed, #{}),

    case Delegate of
        true -> {delegate, Intent, Args, Response};
...
parse_json_payload(Bin) ->
    ...
    case re:run(Bin,
                <<"{.*}">>,
                [dotall, {capture, first, binary}])
        of
        {match, [JsonOnly]} ->
            try jsx:decode(JsonOnly, [return_maps]) catch
                _:_ -> #{}
            end;
        nomatch -> #{}
    end.
...
-spec build_new_context(binary(), binary(),
                        list()) -> list().

build_new_context(Prompt, Response, Prev) ->
    New = Prev ++
              [#{<<"role">> => <<"user">>,
                 <<"content">> => to_bin(Prompt)},
               #{<<"role">> => <<"assistant">>,
                 <<"content">> => to_bin(Response)}],
