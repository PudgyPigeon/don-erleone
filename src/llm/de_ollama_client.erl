...
generate(Prompt, System, Context, Tools, Opts) ->
    ...
    Payload = de_ollama_brain:build_payload(
        maps:get(
            model,
            Opts
        ),
...
-spec message_loop(context(), binary(), term()) ->
    {ok, map()}
    | {error, term()}.

message_loop(
    #{tmo := Tmo, host := Host} = Ctx,
    Buffer,
    Acc
) ->
    receive
        Msg ->
            dispatch(
                handle_stream_event(Msg, Ctx, Buffer, Acc),
                Ctx
            )
...
%% --- Happy Paths ---
handle_stream_event(
    {gun_response, C, R, IsFin, Status, _Headers},
    #{conn := C, ref := R} = Ctx,
    B,
    A
) ->
...
%% --- Error Paths ---
handle_stream_event(
    {gun_error, C, R, Reason},
    #{conn := C, ref := R, host := Host},
    _B,
    _A
) ->
    logger:error(#{
...
handle_stream_event(
    ...
    element(1, GunDown) =:= gun_down,
    element(2, GunDown) =:= C
->
    logger:error(#{
        event => ollama_connection_down,
        host => Host
    }),
...
handle_data(#{is_stream := true} = Ctx, Buffer, Acc) ->
    {NextBuf, NewAcc} =
        de_ollama_client_logic:process_ndjson(
            Buffer,
            Acc,
            maps:get(cb, Ctx)
        ),
...
handle_final_data(
    ...
    {ok,
        extract_message(
            de_ollama_client_logic:finalize(
                Remainder,
                TempAcc,
                CB
            )
