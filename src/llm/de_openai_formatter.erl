...
-spec stream_chunk(cowboy_req:req(), binary(),
                   term()) -> ok.
...
stream_done(Req, MissionId) ->
    ...
    FinishData = #{<<"id">> => IdStr,
                   ...
                   <<"choices">> =>
                       [#{<<"index">> => 0, <<"delta">> => #{},
                          <<"finish_reason">> => <<"stop">>}]},
    FinishPayload = iolist_to_binary(["data: ",
                                      jsx:encode(FinishData),
                                      "\n\n"]),
...
-spec stream_error(cowboy_req:req(), term(),
                   term()) -> ok.
...
mission_id_str(Id) ->
    iolist_to_binary(io_lib:format("~p", [Id])).
