...
accumulate(<<>>, Msg, CB) ->
    ...
    Msg;
accumulate(AccMap, #{<<"message">> := NewMsg}, CB) when
    is_map(AccMap)
->
...
merge_call(undefined, New) ->
    New;
merge_call(Old, undefined) ->
    Old;
merge_call(Old, New) ->
    Merged = maps:merge(Old, New),
    Merged#{
