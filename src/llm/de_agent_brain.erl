...
-spec analyze_loop_step(map()) ->
    {stop, binary()}
    | {continue, list()}.

%% Priority 1: Tool calls always take precedence if present
analyze_loop_step(#{<<"tool_calls">> := Calls}) when
    is_list(Calls), length(Calls) > 0
->
    {continue, Calls};
