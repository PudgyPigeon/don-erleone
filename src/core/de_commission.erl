...
-spec child_specs(de_config:config(),
                  de_config:sub_config()) -> [supervisor:child_spec()].
...
child_specs(Config, SubConfig) ->
    [%% 1. The Underboss (Fleet Manager for Caporegimes)
     ...
     poolboy:child_spec(de_consigliere_pool,
                        pool_config(),
                        [Config])].
...
pool_config() ->
    [{name, {local, de_consigliere_pool}},
     ...
     {max_overflow,
      15},  %% Expanded overflow for busy shifts
     {strategy,
      fifo}].     %% Ensure we don't 'wear out' the first worker in the pool
