...
test_stream_events() ->
  [
    ?_assertEqual({next, <<>>, <<>>},
                  de_ollama_client:handle_stream_event({gun_response, self(), ref, nofin, 200, []}, ?CTX, <<>>, <<>>)),
    ?_assertEqual({error, {http_status, 404}},
                  de_ollama_client:handle_stream_event({gun_response, self(), ref, nofin, 404, []}, ?CTX, <<>>, <<>>)),
    ?_assertEqual({error, {stream_err, reason}},
                  de_ollama_client:handle_stream_event({gun_error, self(), ref, reason}, ?CTX, <<>>, <<>>)),
    ?_assertEqual({error, {gun_err, reason}},
                  de_ollama_client:handle_stream_event({gun_error, self(), reason}, ?CTX, <<>>, <<>>)),
    ?_assertEqual({error, connection_closed},
                  de_ollama_client:handle_stream_event({gun_down, self(), http, closed, [], []}, ?CTX, <<>>, <<>>))
  ].
