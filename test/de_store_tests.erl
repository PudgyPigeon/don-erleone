...
-spec test_post_retrieval() -> ok.

test_post_retrieval() ->
    {ok, Id} = de_store:post_mission(<<"s1">>,
                                     <<"i1">>,
                                     <<"p">>,
                                     []),
...
-spec test_post_initial_status() -> ok.

test_post_initial_status() ->
    {ok, Id} = de_store:post_mission(<<"s2">>,
                                     <<"i2">>,
                                     <<"p">>,
                                     []),
    {ok, Mission} = de_store:get_mission(Id),
    ?assertEqual(pending,
                 (de_store:mission_status(Mission))).

-spec test_update_status() -> ok.

test_update_status() ->
    {ok, Id} = de_store:post_mission(<<"s3">>,
                                     <<"i3">>,
                                     <<"p">>,
                                     []),
    ok = de_store:update_status(Id, in_progress),
    {ok, Mission} = de_store:get_mission(Id),
    ?assertEqual(in_progress,
                 (de_store:mission_status(Mission))).

-spec test_complete_status() -> ok.

test_complete_status() ->
    {ok, Id} = de_store:post_mission(<<"s4">>,
                                     <<"i4">>,
                                     <<"p">>,
                                     []),
...
-spec test_complete_result() -> ok.

test_complete_result() ->
    {ok, Id} = de_store:post_mission(<<"s5">>,
                                     <<"i5">>,
                                     <<"p">>,
                                     []),
    ok = de_store:complete_mission(Id, <<"res">>),
    {ok, Mission} = de_store:get_mission(Id),
    ?assertEqual(<<"res">>,
                 (de_store:mission_result(Mission))).

-spec test_fail_status() -> ok.

test_fail_status() ->
    {ok, Id} = de_store:post_mission(<<"s6">>,
                                     <<"i6">>,
                                     <<"p">>,
                                     []),
...
-spec test_fail_error() -> ok.

test_fail_error() ->
    {ok, Id} = de_store:post_mission(<<"s7">>,
                                     <<"i7">>,
                                     <<"p">>,
                                     []),
    ok = de_store:fail_mission(Id, <<"err">>),
    {ok, Mission} = de_store:get_mission(Id),
    ?assertEqual(<<"err">>,
                 (de_store:mission_error(Mission))).

-spec test_get_latest_context() -> ok.

test_get_latest_context() ->
    Session = <<"ctx_sess">>,
    de_store:post_mission(Session, <<"i">>, <<"p">>, [1]),
    timer:sleep(10),
    de_store:post_mission(Session,
                          <<"i">>,
                          <<"p">>,
                          [1, 2]),
    ?assertEqual([1, 2],
                 (de_store:get_latest_context(Session))).

-spec test_get_pending_missions() -> ok.

test_get_pending_missions() ->
    {ok, Id} = de_store:post_mission(<<"pend">>,
                                     <<"i">>,
                                     <<"p">>,
                                     []),
