...
-spec delegation_integration_test() -> ok.
...
delegation_integration_test() ->
    ...
    meck:expect(de_ollama_client,
                generate,
                4,
                {ok,
                 <<"{\"delegate_required\": false, \"response\": "
                   "\"ok\"}">>}),
    Tag = make_ref(),
    de_consigliere:handle_mission(<<"test_session">>,
                                  <<"Hi">>,
                                  {self(), Tag}),
    Result = receive
                 {Tag, {done, _, _}} -> ok after 5000 -> timeout
             end,
